...
template <>
AWS_ROUTE53DOMAINS_API TLDInMaintenance Route53DomainsError::GetModeledError() {
  assert(this->GetErrorType() == Route53DomainsErrors::T_L_D_IN_MAINTENANCE);
  return TLDInMaintenance(this->GetJsonPayload().View());
}
...
static const int T_L_D_IN_MAINTENANCE_HASH = HashingUtils::HashString("TLDInMaintenance");
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  } else if (hashCode == T_L_D_IN_MAINTENANCE_HASH) {
    return AWSError<CoreErrors>(static_cast<CoreErrors>(Route53DomainsErrors::T_L_D_IN_MAINTENANCE), RetryableType::NOT_RETRYABLE);
